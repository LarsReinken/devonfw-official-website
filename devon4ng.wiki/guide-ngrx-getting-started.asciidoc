...
* the app provides a rich experience with offline functionality, e.g. a native app built with xref:guilde-ionic-getting-started.asciidoc[Ionic]
...
* Plain Rxjs using the simple store described in xref:cookbook-abstract-class-store.asciidoc[Abstract Class Store]
...
[[guide-ngrx-getting-started.asciidoc_setup]]
== Setup
...
[[guide-ngrx-getting-started.asciidoc_concept]]
== Concept
...
Continue with xref:guide-ngrx-simple-store.asciidoc[Creating a Simple Store].